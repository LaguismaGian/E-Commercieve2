                ...
                @auth
                    ...
                    {{-- Admin Dashboard Button --}}
                        @if(Auth::user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="flex items-center justify-center gap-1.5 bg-slate-800 text-white border border-slate-700 p-2 sm:px-4 sm:py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest hover:bg-slate-700 transition-all shadow-sm">
                                
                                {{-- Settings/Dashboard Icon (Visible only on mobile) --}}
                    ...
                    @else
                        {{-- Login / Register --}}
                        <a href="{{ route('login') }}" class="font-sans font-bold text-[10px] uppercase tracking-widest text-gray-400 hover:text-brand-orange transition-colors whitespace-nowrap">
